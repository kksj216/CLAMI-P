...
import edu.handong.csee.isel.util.Utils;
	...
	void prediction(Instances instances,String positiveLabel,boolean isExperimental){
		...
		if(!forCLAMI)
			...
			if (this.inversePercent>0) {
				Utils.getCLAMIResult2(instances,instances,positiveLabel,percentileCutoff,suppress,isExperimental,mlAlg, this.inversePercent);
			}
			else {
